...
export const AppProvider = ({ children }) => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get(`https://randomuser.me/api/?results=50&seed=google`);
  ...
  useEffect(() => {
    if (searchTerm) {
      const fetchData = async () => {
        const result = await axios(`https://randomuser.me/api/?results=50&seed=${searchTerm}`);
